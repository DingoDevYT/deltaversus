...
// ============================================================================
// (ROARING) KNIGHT — "NEW SET" (Gemini recipes). TEST patterns wired only into the
// attack tester + showcase, NOT the Knight's real in-game moveset. Real knight sprites.
// 30fps recipe speeds -> per-tick @60Hz via F (=0.5); absolute delays stay in ticks (s*60).
// ============================================================================
PATTERNS.kx_tracking = {   // KNIFE DANCE: perimeter knives project red lines THROUGH the soul, lock the angle, then
  dur: 520, box: { w: 160, h: 160 },   // slash across. A ring of 8 blades orbits throughout (radius breathes 100<->140).
  tick(a) {
    const { f, box, add, rng, soul } = a;
    const cx = box.x + box.w / 2, cy = box.y + box.h / 2;
    if (f === 0) for (let i = 0; i < 8; i++)                               // PHASE 2 EXTRA: orbiting blade ring
      add({ ...bulletProps('knightknife'), x: cx, y: cy, r: 6, grazeR: 15, scale: 1.15, spin: 0.08 * F, vx: 0, vy: 0, life: 520,
            orbit: { cx, cy, R: 120, w: 0.05 * F, ang: i / 8 * Math.PI * 2, pulse: { base: 120, amp: 20, freq: 0.03 * F } } });
    if (f >= 12 && f < 12 + 21 * 10 && (f - 12) % 21 === 0) {              // 10 tracking knives, one every 0.35s
      const th = rng() * 6.28, px = cx + Math.cos(th) * 180, py = cy + Math.sin(th) * 180;
      const line = { shape: 'line', color: '#f33', len: Math.hypot(box.w, box.h) * 2.6, thick: 3,
                     x: px, y: py, rot: Math.atan2(cy - py, cx - px), vx: 0, vy: 0, tellT: 36, armWindow: 10, dmg: 18, _t: 0, _lock: 24, _shot: 0 };
      line.emit = function (b, out, s) {
        b._t++;
        if (b._t <= b._lock) b.rot = Math.atan2(s.y - b.y, s.x - b.x);     // TRACK the soul for 0.4s, then LOCK the angle (0.2s before the cut)
        if (b.armed && !b._shot) { b._shot = 1;                           // on the cut, a knife streaks along the locked line
          const dx = Math.cos(b.rot), dy = Math.sin(b.rot);
          out.push({ ...bulletProps('knightknife'), x: b.x, y: b.y, vx: dx * 12 * F, vy: dy * 12 * F, rot: b.rot, scale: 1.3, r: 0, noHit: true, life: 70 });
        }
      };
      add(line);
    }
...
PATTERNS.kx_rend = {   // BOARD REND: the board cleaves, halves pull 40px apart, diamond TEETH erupt OUTWARD from
  dur: 660, box: { w: 160, h: 160 },   // the gap, then it snaps shut. Alternates horizontal / vertical, 6 splits.
  tick(a) {
    const { f, box, add, rng } = a;
    const cx = box.x + box.w / 2, cy = box.y + box.h / 2, GAP = 108;
    if (f >= 6 * GAP) return;
    const ev = f % GAP, k = Math.floor(f / GAP), horiz = k % 2 === 0;
    if (ev === 0)                                                          // cut tell
      add({ shape: 'line', color: '#f33', len: (horiz ? box.w : box.h) * 1.05, thick: 4,
            x: cx, y: cy, rot: horiz ? 0 : Math.PI / 2, vx: 0, vy: 0, tellT: 24, armWindow: 8, dmg: 20 });
    let off = 0;                                                          // split: slide 20px each side, hold, snap back
    if (ev >= 24 && ev < 36) off = (ev - 24) / 12 * 20;
    else if (ev >= 36 && ev < 78) off = 20;
    else if (ev >= 78 && ev < 90) off = 20 * (1 - (ev - 78) / 12);
    if (off > 0) a.fx.split = { axis: horiz ? 'h' : 'v', offset: off };
    if (ev === 30) for (let i = 0; i < 6; i++) {                          // teeth erupt outward from the open gap
      const t = (i + 0.5) / 6, sp = (4 + (i % 2) * 3) * F;                // alternating 4 / 7 px/frame
      if (horiz) { const x = box.x + t * box.w;
        add({ ...bulletProps('knighttooth'), x, y: cy - 6, vx: 0, vy: -sp, rot: 0, scale: 0.8, r: 6, grazeR: 12, ridesSplit: 1 });
        add({ ...bulletProps('knighttooth'), x, y: cy + 6, vx: 0, vy: sp, rot: Math.PI, scale: 0.8, r: 6, grazeR: 12, ridesSplit: -1 });
      } else { const y = box.y + t * box.h;
        add({ ...bulletProps('knighttooth'), x: cx - 6, y, vx: -sp, vy: 0, rot: -Math.PI / 2, scale: 0.8, r: 6, grazeR: 12, ridesSplit: 1 });
        add({ ...bulletProps('knighttooth'), x: cx + 6, y, vx: sp, vy: 0, rot: Math.PI / 2, scale: 0.8, r: 6, grazeR: 12, ridesSplit: -1 });
      }
    }
  },
};
PATTERNS.kx_tunnel = {   // BLADE HALLWAY: a WIDE arena; twin walls of crescent blades sweep in from the RIGHT with a
  dur: 470, box: { w: 300, h: 160 },   // sine-wave safe corridor, then halt, rotate, and lattice-slash across.
  tick(a) {
    const { f, box, add } = a;
    const cx = box.x + box.w / 2, cy = box.y + box.h / 2;
    if (f >= 24 && f < 264 && (f - 24) % 9 === 0) {                       // continuous moving tunnel from the right
      const gapC = cy + 35 * Math.sin(0.08 * f * 2), gapH = 46, spd = -4 * F, x = box.x + box.w + 14;
      for (let yy = box.y - 10; yy <= box.y + box.h + 10; yy += 16) {
        if (Math.abs(yy - gapC) < gapH / 2) continue;                     // leave the moving corridor open
        add({ shape: 'crescent', color: '#fff', x, y: yy, vx: spd, vy: 0, r: 9, grazeR: 16, rot: Math.PI, scale: 1.0, life: 200 });
      }
    }
    if (f === 300) for (let i = 0; i < 6; i++) {                          // LATTICE: red tells rotate in, then dash across
      const gy = box.y + box.h * (i + 0.5) / 6;
      add({ shape: 'line', color: '#f33', len: box.w * 1.2, thick: 5, x: cx, y: gy, rot: 0, vx: 0, vy: 0, tellT: 40, armWindow: 12, dmg: 22 });
    }
    if (f === 300) for (let i = 0; i < 4; i++) {
      const gx = box.x + box.w * (i + 0.5) / 4;
      add({ shape: 'line', color: '#f33', len: box.h * 1.2, thick: 5, x: gx, y: cy, rot: Math.PI / 2, vx: 0, vy: 0, tellT: 40, armWindow: 12, dmg: 22 });
    }
  },
};
PATTERNS.kx_stars = {   // CRYSTAL BARRAGE: crystal stars fall, decelerate to a dead stop, drift UP, then burst into a
  dur: 560, box: { w: 160, h: 160 },   // 6-way shard spread. Four waves, each falling a little faster.
  tick(a) {
    const { f, box, add, rng } = a;
    const WAVE = 120;
    if (f % WAVE === 0 && f < 480) {
      const spd = (6 + Math.floor(f / WAVE)) * F;                         // later waves drop faster
      for (let i = 0; i < 4; i++) {
        const x = box.x + box.w * (0.12 + rng() * 0.76);
        add({ ...bulletProps('knightstar'), x, y: box.y - 12, vx: 0, vy: spd, ay: -0.2 * F, r: 8, grazeR: 15, scale: 1.05, spin: 0.05 * F,
              burst: 46, burstN: 6, burstImg: 'knighttri', burstSpeed: 5 * F, burstScale: 0.7, burstR: 5, burstLife: 100 });   // decel -> stop -> drift up -> 6-way burst
      }
    }
  },
};
PATTERNS.kx_sweep = {   // SANGUINE SLASH: warning lines pivot around CENTRE, lock, and slash — 1,2,3,4 of them —
  dur: 620, box: { w: 160, h: 160 },   // then a 4-beam cross spins a full 360 as a live hazard. Screen dims.
  tick(a) {
    const { f, box, add, rng } = a;
    const cx = box.x + box.w / 2, cy = box.y + box.h / 2, LEN = Math.hypot(box.w, box.h) * 2.2;
    a.fx.blackout = true;
    for (const [t, n] of [[0, 1], [48, 2], [96, 3], [144, 4]]) if (f === t) {   // escalating intersecting strikes, 0.8s apart
      const base = rng() * Math.PI, dir = rng() < 0.5 ? 1 : -1;
      for (let i = 0; i < n; i++)
        add({ shape: 'line', color: '#f33', len: LEN, thick: 7, x: cx, y: cy, rot: base + i * Math.PI / n,
              vx: 0, vy: 0, spin: dir * 0.09, spinDecay: 0.92, tellT: 40, armWindow: 10, dmg: 24 });
    }
    if (f === 230) for (let i = 0; i < 4; i++)                            // ROTATING CROSS: continuous spinning hazard (~full 360 over 3s)
      add({ shape: 'line', color: '#f33', len: LEN, thick: 6, x: cx, y: cy, rot: i * Math.PI / 2,
            vx: 0, vy: 0, spin: 0.035, tellT: 20, armWindow: 190, dmg: 22, shakeOnCut: true });
  },
};
PATTERNS.kx_ultimate = {   // THE ROARING (ult): full-screen arena, the Knight drags the soul with a suction pull while a
  dur: 900, box: { w: 160, h: 160 },   // star storm reverses + detonates, ending in a screen-cleaving giant blade (30px safe floor).
  tick(a) {
    const { f, box, tier, add, rng } = a;
    a.fx.arena = true; a.fx.bgStars = true;
    const cx = box.x + box.w / 2, cy = box.y + box.h / 2, kx = box.x + box.w + 180, ky = cy;   // Knight looms right of the box
    if (f >= 30 && f < 540) a.fx.pull = { x: kx, y: ky, force: 1.8 * F };  // suction toward the Knight (still escapable)
    a.fx.boss = { key: (Math.floor(f / 10) % 2) ? 'knightroar1' : 'knightroar0', x: kx - 40, y: ky - 40, scale: 2.4 };
    if (f >= 40 && f < 520 && every(f, rate(8, tier)))                     // STAR STORM: fall, decelerate, reverse UP, burst into 4 shards
      add({ ...bulletProps('knightstar'), x: cx + (rng() - 0.5) * 640, y: box.y - 120, vx: 0, vy: 7 * F, ay: -0.22 * F, r: 8, grazeR: 15, scale: 1.0, spin: 0.05 * F,
            burst: 46, burstN: 4, burstImg: 'knighttri', burstSpeed: 4 * F, burstScale: 0.7, burstR: 5, burstLife: 90 });
    // GIANT CLEAVE: a huge blade slams down, hazard covers everything above a 30px safe floor (thick horizontal line hitbox)
    if (f === 640) { const floorY = box.y + box.h + 62, T = 900;
      add({ shape: 'line', color: '#f33', len: 900, thick: T, x: cx, y: floorY - T / 2, rot: 0, vx: 0, vy: 0, tellT: 46, armWindow: 30, dmg: 45, shakeOnCut: true }); }
    if (f > 686 && f < 712) { a.fx.whiteout = Math.min(1, (f - 686) / 20); a.fx.shake = 8; }   // impact flash
  },
};
