...
const ASSET_V = 54;   // bump when sprite/manifest files change so browsers refetch
A.load = function (done) {
  fetch('assets/manifest.json?a=' + ASSET_V).then(r => r.json()).then(man => {
    ...
    for (const ch of ['kris', 'susie', 'ralsei', 'noelle', 'lancer', 'berdly', 'jevil', 'spamton', 'knight', 'gerson'])
